...
from evals.schemas import DatasetEvaluationResult, BenchmarkResult
...
def export_benchmark_result_json(result: BenchmarkResult, output_path: str | Path) -> None:
    """
    Exports a BenchmarkResult object to a deterministic, formatted JSON file.
    Ensures stable sort keys and standardized 2-space indentation.
    """
    path = Path(output_path)
    path.parent.mkdir(parents=True, exist_ok=True)
