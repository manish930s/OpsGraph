...
The same `GEMINI_API_KEY` may be used to authenticate both the Gemini Embedding pipeline and the LLM Gateway generation workloads. However, the model configurations are strictly separated. The LLM Gateway is configured via `settings.GEMINI_MODEL` (e.g. `gemini-2.5-flash`), which is validated at runtime to be distinct from `settings.GEMINI_EMBEDDING_MODEL` using a Pydantic Settings model validator.
